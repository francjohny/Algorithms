...
public class PathQueries {
    ...
    public static void main(String[] args) {
        ...
        dfs(1, 0);
        SegmentTree segmentTree = new SegmentTree(1, 20005);
        for (int i = 1; i <= n; i++) {
            segmentTree.update(start[i], values[i]);
            segmentTree.update(end[i], -values[i]);
        }
        for (int i = 0; i < q; i++) {
            ...
            if (query == 1) {
                int a = fastIO.getInt();
                int b = fastIO.getInt();
                segmentTree.update(start[a], b);
                segmentTree.update(end[a], -b);
            } else {
                // query = 2
                int s = fastIO.getInt();
                out.println(segmentTree.query(start[1], end[s] - 1));
            }
        }
        out.close();
    }
