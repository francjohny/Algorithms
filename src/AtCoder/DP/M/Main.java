...
public class Main {
    ...
    private static long getNumberOfDifferentWaysToDistributeCandies(int n, int K, int[] a) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= K; j++) {
                // https://codeforces.com/blog/entry/97435
                // Alt: https://atcoder.jp/contests/dp/submissions/4033538
                dp[i][j] = add(((j - 1 >= 0 ? dp[i][j - 1] : 0) + dp[i - 1][j]), (j - (a[i - 1] + 1)) >= 0 ? MOD - dp[i - 1][j - (a[i - 1] + 1)] : 0);
            }
